...
public class TreeViewWithTreeModel<T> : TreeView where T : TreeElement
{
    ...
    public TreeModel<T> CustomTreeModel { get { return m_TreeModel; } }
    ...
    protected override TreeViewItem BuildRoot()
    {
        ...
        return new TreeViewItem<T>(m_TreeModel.root.ElementID, depthForHiddenRoot, m_TreeModel.root.name, m_TreeModel.root);
    }
    ...
    void AddChildrenRecursive(T parent, int depth, IList<TreeViewItem> newRows)
    {
        ...
        {
            var item = new TreeViewItem<T>(child.ElementID, depth, child.name, child);
            newRows.Add(item);
    ...
    void Search(T searchFromThis, string search, List<TreeViewItem> result)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            {
                result.Add(new TreeViewItem<T>(current.ElementID, kItemDepth, current.name, current));
            }
    ...
    //    var selectedIDs = draggedElements.Select(x => x.ElementID).ToArray();
